...
public class Turret : MonoBehaviour, Plot.IUpgradeable
{
    [Header("References")]
    ...
    private void Shoot()
    {
        ...
        Bullet bulletScript = bulletObj.GetComponent<Bullet>(); // change this to the bullet script or objectname you are using
        bulletScript.SetTarget(target); // send target over to the bullet script
    ...
    {
        if (CalculateUpgradeCost() > LevelManager.main.silver) // Check if you got enough gold to upgrade the turret
        {
            Debug.Log("Not enough silver to upgrade the turret!"); // 
            CloseUpgradeUI();
            return;
        }
        LevelManager.main.SpendCurrencySilver(CalculateUpgradeCost()); // Spend the gold to upgrade the turret
        ...
        bulletDamage = bulletDamage + 0.15f; // Calculate the new bullet damage
        // bulletSpeed = bulletSpeed + 0.1f; // Calculate the new bullet speed
        // Upgrade the turret
        Debug.Log("Turret upgraded!");
        Debug.Log("New BPS: " + bps);
        Debug.Log("New Range: " + targetingRange);
        Debug.Log("Upgrade cost: " + CalculateUpgradeCost());
        CloseUpgradeUI();
        UpdateUpgradeCostText();
    }
    ...
    public void SellTurret()
    {
        ...
        int turretPrice = BuildManager.main.GetTurretDataByName(turretName).silver;

        if (turretPrice <= 0)
        {
            Debug.LogError("Turret data not found or invalid price!");
            Debug.LogError("Are you sure you named the Prefab the same as the shop item?");
            Debug.LogError("Did you try to sell a Turret for 0 cost? This is not allowed.");

            return;
        }

        // // Refund the silver price of the turret (50% of total rpice)
        LevelManager.main.IncreaseCurrencySilver(turretPrice/2);
    ...
    private int CalculateUpgradeCost()
    {
        ...
        Debug.Log("Upgrade cost: " + Mathf.RoundToInt(upgradeCost * Mathf.Pow(level, 0.8f)));
